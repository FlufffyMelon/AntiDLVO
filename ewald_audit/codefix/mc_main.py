...
def run_simulation(cfg):
    ...
    _print_memory("run_simulation START")
    # Create components
    units = create_units(cfg)
    _print_memory("After create_units")
    system = create_system(cfg, units)
    _print_memory("After create_system")
    topology = create_topology(cfg, units)
    _print_memory("After create_topology")
    setup_initial_configuration(
        system,
        cfg.get("types", []),
        topology,
    ...
    _print_memory("After setup_initial_configuration")
    logger = create_logger(cfg)
    sampler = create_sampler(system, topology, cfg, logger, units)
    _print_memory("After create_sampler")
    ...
    try:
        ...
        for i in range(chunks):
            ...
            _print_memory(f"Before MC chunk {i+1}/{chunks} ({take} steps)")
            sampler.move(take)
            steps_remaining -= take
            _print_memory(f"After MC chunk {i+1}/{chunks} (N_atoms={system.N_atoms}, N_molecules={system.N_molecules})")
            if bar is not None:
                bar.update(take)
        if bar is not None:
            bar.close()
        ...
        if final_energy is not None:
            ...
            print("Final energy: N/A (cache invalidated)")
        if final_forces is not None and final_forces.ndim == 2:
            print(
        ...
        print(f"Final number of molecules: {system.N_molecules}")
        ...
        if recomputed_forces is not None:
            print(
        ...
        if (
            final_forces is not None
            and recomputed_forces is not None
            and final_forces.shape == recomputed_forces.shape
        ):
            print(
        ...
        logger.log_info(
            f"Final energy (recomputed): {recomputed_energy:.6f} {units.energy_label}; "
            f"delta vs cached: {(recomputed_energy - (final_energy or 0.0)):.6f} {units.energy_label}"
        )
