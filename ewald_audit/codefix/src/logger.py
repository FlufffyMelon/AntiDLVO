...
class Logger:
    ...
    def __init__(
        ...
        overwrite: bool = True,
        units: Units = None,
        insertable_type_names: Optional[List[str]] = None,
    ):
        """
        ...
        Args:
            ...
            insertable_type_names: Ordered list of insertable type names for
                per-type count columns (muVT).
        """
        ...
        self.insertable_type_names: List[str] = list(insertable_type_names or [])
    ...
    def _init_log_file(self) -> None:
        ...
        with open(self.log_file, "w") as f:
            ...
            f.write(
                "Columns: Step, Time(s), N_atoms, E_pot, E_tail, E_total, Temperature, Volume, Density, P_id, P_vir, P_tail, P_total, Mu_id, Total_charge, Acceptance_ratio, Action_ratios\n"
            )
    ...
    def log_step(
        ...
        _, _, _, _, _, charges, _, _ = system.get_active_atoms()
        ...
        if sampler:
            ...
            # Auto-detect action keys on first call
            if not self._action_keys:
                self._action_keys = [k for k in ratios.keys() if k != "total"]
        ...
        line = (
            ...
            f"{density:>12.6f} {pressure_ideal:>12.6f} {pressure_virial:>12.6f} {pressure_tail:>12.6f} {pressure_total:>12.6f} {mu_id:>15.6e} {total_charge:>15.6e} {acceptance_total:>12.4f} {action_ratios}"
        )
        ...
        # Per-type molecule counts (muVT)
        type_counts: Dict[str, int] = {}
        for tname in self.insertable_type_names:
            type_counts[tname] = system.get_molecule_count_by_type(tname)
        ...
        if not self._data_header_written:
            ...
            # Per-type count columns
            for tname in self.insertable_type_names:
                base_cols.append(f"n_{tname}")
        ...
        for tname in self.insertable_type_names:
            row_map[f"n_{tname}"] = type_counts.get(tname, 0)
        for k in self._action_keys:
