...
class Topology:
    ...
    def _single_pair_energy_forces(
        self,
        idx: int,
        position: np.ndarray,
        atom_type: int,
        ...
        """
        positions, forces, _, types, _, charges, _, _ = system.get_active_atoms()
        if system.N_atoms == 0:
        ...
        if indices is not None:
            indices_arr = np.atleast_1d(indices).astype(int, copy=False).ravel()
            if np.any(indices_arr < 0) or np.any(indices_arr >= N_atoms):
                raise IndexError("indices contain out-of-range atom ids")
            if intermol:
                mask_valid = np.zeros(N_atoms, dtype=bool)
                mask_valid[indices_arr] = True
            else:
                mask_valid = np.ones(N_atoms, dtype=bool)
                mask_valid[indices_arr] = False
        else:
            if intermol:
                raise ValueError(
        ...
        types_j = types[mask_valid].astype(int, copy=False)
        charges_j = charges[mask_valid]
        n_valid = len(pos_j)

        if n_valid == 0:
            return 0.0, np.zeros((system.capacity, 3), dtype=positions.dtype)
        ...
        # Initialize outputs
        energy_scalar = 0.0
        # Only allocate forces_i for capacity (needed for indexing), but we'll only modify specific indices
        forces_i = np.zeros((system.capacity, 3), dtype=positions.dtype)

        # Process pairwise interactions
        # Use np.unique with return_inverse=False for better performance
        unique_types = np.unique(types_j)
        # Indices of valid atoms in the full system arrays (compute once)
        idx_valid = np.where(mask_valid)[0]
        t1 = int(atom_type)
        for t2 in unique_types:
            forces = self.get_interactions(t1, int(t2))
            if not forces:
                continue

            # Mask for this neighbor type (within valid atoms)
            mask_t = types_j == t2
            if not np.any(mask_t):
                continue
            dr_ij_t = dr_ij[mask_t]
            charges_t = charges_j[mask_t]
            # Map mask_t (over valid atoms) back to global indices
            global_indices_t = idx_valid[mask_t]
            ...
            for f in forces:
                ...
                if np.isinf(energy):
                    return np.inf, np.zeros((system.capacity, 3), dtype=positions.dtype)

                energy_scalar += energy
                # virial_scalar += np.sum(forces_matrix[: dr_ij_t.shape[0], :] * dr_ij_t)
                # Force on i atom
                forces_i[idx] += np.sum(forces_matrix, axis=0)
                # Force on all j atoms following third Newton's law
                # Use pre-computed global indices
                forces_i[global_indices_t, :] += -forces_matrix

        return energy_scalar, forces_i
    ...
    def _single_one_body_energy_forces(
        ...
        """
        positions, _, _, _, _, _, _, _ = system.get_active_atoms()
        energy_scalar = 0.0
        # force_vec = np.zeros(3)
        # Size from capacity to match other force arrays
        force_matrix = np.zeros((system.capacity, 3), dtype=positions.dtype)

        t_i = int(atom_type)
        for ob in self.one_body_forces.get(t_i, []):
            energy, forces = ob(position, t_i, float(charge), system=system)
            if np.isinf(energy):
                return np.inf, np.zeros((system.capacity, 3), dtype=positions.dtype)

            energy_scalar += energy
            force_matrix[idx, :] += np.sum(forces, axis=0)

        return energy_scalar, force_matrix

    def _molecule_energy_forces(
        self,
        ...
        """
        positions_all, _, molecule_ids, types, _, charges, _, _ = system.get_active_atoms()

        energy_scalar = 0.0
        # Initialize with capacity size to match force arrays from helper methods
        forces_on_mol = np.zeros((system.capacity, 3), dtype=positions_all.dtype)
        # virial_scalar = 0.0
        ...
        for i, idx in enumerate(indices):
            ...
            # pair_e, pair_force, pair_w = self._single_pair_energy_forces(
            pair_e, pair_forces = self._single_pair_energy_forces(
                idx,
                positions[i],
                types[idx],
                charges[idx],
            ...
            intermol_e, intermol_forces = self._single_pair_energy_forces(
                idx,
                positions[i],
                types[idx],
                charges[idx],
            ...
            one_e, one_forces = self._single_one_body_energy_forces(
                idx, positions[i], types[idx], charges[idx], system
            )
            ...
            forces_on_mol += pair_forces + intermol_forces + one_forces
            # virial_scalar += pair_w + intermol_w

            if np.isinf(energy_scalar):
                if new_positions is not None:
                    system.positions[indices, :] = old_positions

                return np.inf, np.zeros((system.capacity, 3), dtype=positions_all.dtype)

        if new_positions is not None:
            system.positions[indices, :] = old_positions

        return energy_scalar, forces_on_mol
    ...
    def recompute_caches(self, system: System) -> None:
        ...
        """
        energy, forces = self.compute_energy_forces(system)
        system.potential_energy = energy
        system.forces = forces

    def _total_molecule_energy_forces(
        self, system: System, molecule_indices: Optional[np.ndarray] = None
    ) -> Tuple[float, np.ndarray]:
        """
        Total energy and virial using j > i optimization.
        Returns (energy_total, forces_total).
        """
        if system.N_atoms <= 1:
            return 0.0, np.zeros((system.N_atoms, 3))

        _, forces, molecule_ids, types, _, charges, _, _ = system.get_active_atoms()
        energy_total = 0.0
        # Initialize forces_total with capacity size to match _molecule_energy_forces output
        forces_total = np.zeros((system.capacity, 3), dtype=forces.dtype if forces is not None else np.float64)
        ...
        for mol_idx in molecule_indices:
            ...
            # energy, _, virial = self._molecule_energy_forces(
            energy, forces = self._molecule_energy_forces(
                indices,
                types[indices],
                charges[indices],
                system,
            ...
            if np.isinf(energy):
                return np.inf, np.zeros((system.capacity, 3), dtype=forces.dtype)
    ...
    def compute_energy_forces(self, system: System) -> Tuple[float, np.ndarray]:
        """Compute total potential energy and virial using split pair/one-body paths."""
        energy, forces = self._total_molecule_energy_forces(system)

        if np.isinf(energy):
            return np.inf, np.zeros((system.capacity, 3), dtype=forces.dtype)

        # Add Ewald k-space, self-energy, and dipole correction contributions if enabled
        positions, _, _, _, _, charges, _, _ = system.get_active_atoms()
        ewald_energy = 0.0
        ewald_forces = np.zeros_like(forces)
        if system.ewald_handler:
            # K-space contribution
            kspace_energy, kspace_forces = (
                system.ewald_handler.compute_total_kspace_energy_forces(system.N_atoms)
            )
            ...
            dipole_energy, dipole_forces = (
                system.ewald_handler.compute_dipole_correction_energy_forces(
                    positions, charges, system.get_volume()
                )
            )

            # Pad Ewald forces to capacity size before adding
            # kspace_forces and dipole_forces have shape (N_atoms, 3)
            # ewald_forces has shape (capacity, 3)
            if kspace_forces.shape[0] < system.capacity:
                padded_kspace = np.zeros((system.capacity, 3), dtype=kspace_forces.dtype)
                padded_kspace[:kspace_forces.shape[0]] = kspace_forces
                kspace_forces = padded_kspace
            if dipole_forces.shape[0] < system.capacity:
                padded_dipole = np.zeros((system.capacity, 3), dtype=dipole_forces.dtype)
                padded_dipole[:dipole_forces.shape[0]] = dipole_forces
                dipole_forces = padded_dipole

            ewald_energy += kspace_energy + dipole_energy
            ewald_forces += kspace_forces + dipole_forces

        return energy + ewald_energy, forces + ewald_forces
    ...
    def get_energy_difference_translation(
        ...
        positions, forces, molecule_ids, types, names, charges, masses, statics = (
            system.get_active_atoms()
        )
        old_positions = positions[atom_indices, :].copy()

        # Energy with old position
        energy_before, forces_before = self._molecule_energy_forces(
            atom_indices,
            types[atom_indices],
        ...
        energy_after, forces_after = self._molecule_energy_forces(
            atom_indices,
            types[atom_indices],
            charges[atom_indices],
            system,
        ...
        delta_forces = forces_after - forces_before

        delta_S_c, delta_S_s = 0.0, 0.0

        if system.ewald_handler:
            # K-space contribution
            delta_kspace_energy, delta_kspace_forces, delta_S_c, delta_S_s = (
                system.ewald_handler.delta_kspace_energy_forces(
                    atom_indices,
                    positions,
                    new_positions,
                    charges[atom_indices],
            ...
            #     system.get_volume(),
            # )
            delta_dipole_energy, delta_dipole_forces = (
                system.ewald_handler.delta_dipole_correction_energy_forces(
                    positions,
                    charges,
                    atom_indices,
                    new_positions,
            ...
            # Pad Ewald forces to capacity size before adding
            # delta_kspace_forces and delta_dipole_forces have shape (N_atoms, 3)
            # delta_forces has shape (capacity, 3)
            if delta_kspace_forces.shape[0] < system.capacity:
                padded_kspace = np.zeros((system.capacity, 3), dtype=delta_kspace_forces.dtype)
                padded_kspace[:delta_kspace_forces.shape[0]] = delta_kspace_forces
                delta_kspace_forces = padded_kspace
            if delta_dipole_forces.shape[0] < system.capacity:
                padded_dipole = np.zeros((system.capacity, 3), dtype=delta_dipole_forces.dtype)
                padded_dipole[:delta_dipole_forces.shape[0]] = delta_dipole_forces
                delta_dipole_forces = padded_dipole

            delta_energy += delta_kspace_energy + delta_dipole_energy
            delta_forces += delta_kspace_forces + delta_dipole_forces

        return delta_energy, delta_forces, delta_S_c, delta_S_s
    ...
    def get_energy_insertion(
        ...
        """
        Compute energy change for inserting a molecule whose atoms have already
        been temporarily appended to the system arrays at ``atom_indices``.

        The Ewald structure factors have **not** been updated yet – they still
        describe the old system, which is exactly what the delta formula needs.

        Returns:
            (delta_energy, new_Sc, new_Ss)
            * delta_energy – total energy change (real + kspace + dipole)
            * new_Sc, new_Ss – (K, M) structure factor columns to append after
              acceptance (None when Ewald is disabled).
        """
        positions, _, _, types, _, charges, _, _ = system.get_active_atoms()
        mol_charges = charges[atom_indices]
        mol_positions = positions[atom_indices]

        # 1) Real-space pair + intramolecular + one-body
        delta_energy, _ = self._molecule_energy_forces(
            atom_indices,
            types[atom_indices],
            mol_charges,
            system,
        ...
        if np.isinf(delta_energy):
            return np.inf, None, None

        new_Sc, new_Ss = None, None

        if system.ewald_handler:
            # 2) K-space delta (using OLD structure factor sums)
            # n_atoms before insertion (molecule is already temporarily added)
            n_atoms_before = system.N_atoms - len(atom_indices)
            dU_k, new_Sc, new_Ss = (
                system.ewald_handler.delta_kspace_energy_insertion(
                    mol_positions, mol_charges, n_atoms_before
                )
            )
            delta_energy += dU_k

            # 3) Dipole correction delta
            if system.ewald_handler.dipole_correction:
                # "After": all current atoms (includes new ones)
                e_after, _ = (
                    system.ewald_handler.compute_dipole_correction_energy_forces(
                        positions, charges, system.get_volume()
                    )
                )
                # "Before": without the new atoms
                old_mask = np.ones(system.N_atoms, dtype=bool)
                old_mask[atom_indices] = False
                e_before, _ = (
                    system.ewald_handler.compute_dipole_correction_energy_forces(
                        positions[old_mask], charges[old_mask], system.get_volume()
                    )
                )
                delta_energy += e_after - e_before

        return delta_energy, new_Sc, new_Ss

    def get_energy_deletion(
        self,
        atom_indices: np.ndarray,
        system: System,
    ) -> float:
        """
        Compute energy change for removing a molecule at ``atom_indices``.

        The molecule is **still present** in the system arrays when this method
        is called.

        Returns:
            delta_energy – the (negative) energy change upon removal.
        """
        positions, _, _, types, _, charges, _, _ = system.get_active_atoms()
        mol_charges = charges[atom_indices]

        # 1) Real-space pair + intramolecular + one-body interaction with rest
        e_mol, _ = self._molecule_energy_forces(
            atom_indices,
            types[atom_indices],
            mol_charges,
            system,
        ...
        if np.isinf(e_mol):
            # Should not normally happen for deletion
            return -np.inf

        # Removing the molecule removes its interaction energy
        delta_energy = -e_mol

        if system.ewald_handler:
            # 2) K-space delta (removing columns from structure factors)
            # n_atoms before deletion (molecule is still present)
            delta_energy += system.ewald_handler.delta_kspace_energy_deletion(
                atom_indices, system.N_atoms
            )

            # 3) Dipole correction delta
            if system.ewald_handler.dipole_correction:
                # "Before" (current, with molecule)
                e_before, _ = (
                    system.ewald_handler.compute_dipole_correction_energy_forces(
                        positions, charges, system.get_volume()
                    )
                )
                # "After" (without molecule)
                keep_mask = np.ones(system.N_atoms, dtype=bool)
                keep_mask[atom_indices] = False
                e_after, _ = (
                    system.ewald_handler.compute_dipole_correction_energy_forces(
                        positions[keep_mask],
                        charges[keep_mask],
    ...
    def compute_solvation_force(self, system: System) -> Optional[float]:
        ...
        for t_id, flist in self.one_body_forces.items():
            walls_10_4_3 = [f for f in flist if isinstance(f, Wall_10_4_3)]
            walls_electric = [f for f in flist if isinstance(f, HardWall)]

            if walls_10_4_3 or walls_electric:
        ...
        if walls_10_4_3:
            ...
            bottom_mask = names == "Wb"
            top_mask = names == "Wt"

            bottom_forces = forces[bottom_mask, 2]
            top_forces = forces[top_mask, 2]
            f_s = (np.sum(bottom_forces) - np.sum(top_forces)) / 2
