#!/usr/bin/env python3
"""
Test 5: Two coaxial antiparallel dipoles at varying distances.
...
class TestCoaxialAntiparallelDipoles(BaseEwaldTest):
    """Test for two coaxial antiparallel dipoles at varying distances."""

    def __init__(
        self,
        config_file: str = "configs/test_5_coaxial_antiparallel_dipoles.yaml",
        **kwargs,
    ):
        """Initialize the coaxial antiparallel dipoles test."""
        super().__init__(config_file, **kwargs)
        self.test_name = "Coaxial Antiparallel Dipoles"

    def run_test(self) -> Dict[str, Any]:
        """Run the coaxial antiparallel dipoles test."""
        ...
        for distance in distances:
            comp_energy, real_energy = self._setup_coaxial_antiparallel_dipoles(
        ...
        self._create_energy_plot(
            ...
            computed_energies,
            real_energies,
            "Coaxial Antiparallel Dipoles: Ewald vs Coulomb",
            "test_5_coaxial_antiparallel_dipoles.png",
        )
    ...
    def _setup_coaxial_antiparallel_dipoles(
        self, distance: float, dipole_length: float
    ) -> tuple[float, float]:
        """Setup two coaxial antiparallel dipoles at specified distance."""
        ...
        # Real energy: Coulomb interaction between two coaxial antiparallel dipoles
        # For coaxial antiparallel dipoles: E = k*q²*(1/r + 1/(r+l) + 1/(r-l) - 1/(r+2l))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test 5: Coaxial Antiparallel Dipoles")
    parser.add_argument(
        "--config",
        default="configs/test_5_coaxial_antiparallel_dipoles.yaml",
    ...
    print("=" * 60)
    print("TEST 5: COAXIAL ANTIPARALLEL DIPOLES")
    print("=" * 60)

    try:
        test = TestCoaxialAntiparallelDipoles(
        ...
        print(
            f"Plot saved to: {test.results_dir / 'test_5_coaxial_antiparallel_dipoles.png'}"
        )
