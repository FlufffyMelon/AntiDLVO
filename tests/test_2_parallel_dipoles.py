#!/usr/bin/env python3
"""
Test 2: Two parallel dipoles at varying distances.
...
class TestParallelDipoles(BaseEwaldTest):
    """Test for two parallel dipoles at varying distances."""

    def __init__(
        self, config_file: str = "configs/test_2_parallel_dipoles.yaml", **kwargs
    ):
        """Initialize the parallel dipoles test."""
        super().__init__(config_file, **kwargs)
        self.test_name = "Parallel Dipoles"

    def run_test(self) -> Dict[str, Any]:
        """Run the parallel dipoles test."""
        ...
        for distance in distances:
            comp_energy, real_energy = self._setup_parallel_dipoles(
        ...
        self._create_energy_plot(
            ...
            computed_energies,
            real_energies,
            "Parallel Dipoles: Ewald vs Coulomb",
            "test_2_parallel_dipoles.png",
        )
    ...
    def _setup_parallel_dipoles(
        self, distance: float, dipole_length: float
    ) -> tuple[float, float]:
        """Setup two parallel dipoles at specified distance."""
        ...
        # Real energy: Coulomb interaction between two parallel dipoles
        # For parallel dipoles: E = k*q²*(1/r - 1/l - 1/(r^2 + l^2)^0.5)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test 2: Parallel Dipoles")
    parser.add_argument(
        "--config",
        default="configs/test_2_parallel_dipoles.yaml",
    ...
    print("=" * 60)
    print("TEST 2: PARALLEL DIPOLES")
    print("=" * 60)

    try:
        test = TestParallelDipoles(
        ...
        print(f"Plot saved to: {test.results_dir / 'test_2_parallel_dipoles.png'}")
