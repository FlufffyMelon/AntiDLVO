...
def create_system(cfg, units: Units) -> System:
    ...
    mu_raw = cfg.simulation.get("mu") if ensemble == "muVT" else None
    # Resolve mu to a plain dict if it's an OmegaConf container
    if mu_raw is not None and isinstance(mu_raw, (DictConfig, dict)):
        mu = OmegaConf.to_container(mu_raw, resolve=True) if isinstance(mu_raw, DictConfig) else dict(mu_raw)
    else:
        mu = mu_raw
...
def setup_initial_configuration(
    ...
    if cfg is not None and "system" in cfg and "non_static_init" in cfg.system:
        ...
        min_dist_val = cfg.system.non_static_init.get("min_distance")
        if min_dist_val is not None:
            min_distance = float(min_dist_val)
    ...
    if non_static_init_mode == "random_hs":
        _init_random_hs_configuration(
            system, processed_non_static_specs, type_name_to_id, cfg
        )
    elif non_static_init_mode == "uniform":
        ...
        if min_distance is None:
            raise ValueError(
                "system.non_static_init.min_distance must be set for mode 'random'"
            )
...
def _molecule_type_name(molecule_type: str, spec: dict) -> str:
    """Derive a consistent type name for a molecule specification.

    Particle → spec["type"]  (e.g. "Na")
    Dipole   → spec["type_plus"]  (e.g. "Ip")
    """
    if molecule_type == "Particle":
        return spec.get("type", molecule_type)
    elif molecule_type == "Dipole":
        return spec.get("type_plus", molecule_type)
    ...
    for molecule_type, spec, count in processed_static_specs:
        ...
        for i in range(n):
            for j in range(n):
                ...
                system.add_molecule(molecule, type_name=_molecule_type_name(molecule_type, spec))
        ...
        for j in range(lattice_dim):
            for k in range(lattice_dim):
                ...
                for basis_idx in range(2):
                    ...
                    system.add_molecule(molecule, type_name=_molecule_type_name("Particle", spec))
...
def _init_random_configuration(
    ...
    for i, (molecule_type, spec, count) in enumerate(processed_specs):
        ...
        for _ in tqdm(range(count), desc="Count"):
            ...
            if not placed:
                raise RuntimeError(
                    f"Failed to place molecule {_molecule_type_name(molecule_type, spec)} "
                    f"({molecule_type}) after {max_attempts} attempts with "
                    f"min_distance={min_distance} and padding={padding}. "
                    f"Consider system.non_static_init.mode='random_hs'."
                )
...
# ---------------------------------------------------------------------------
# "random_hs" initial configuration: per-pair hard-sphere contact distances and
# per-type hard-wall slabs, taken straight from the force field definition.
# ---------------------------------------------------------------------------

_HS_FORCE_KINDS = ("hard_sphere", "hard_sphere_potential")


def _parse_one_body_type_names(type_spec) -> List[str]:
    """Parse a one_body 'type' entry: "Na", "(Na, Cl)" or a list of names."""
    if isinstance(type_spec, str):
        type_spec = type_spec.strip()
        if type_spec.startswith("(") and type_spec.endswith(")"):
            return [t.strip() for t in type_spec[1:-1].split(",")]
        return [type_spec]
    if isinstance(type_spec, (list, tuple, ListConfig)):
        return [str(t) for t in type_spec]
    return [str(type_spec)]


def _build_hs_contact_matrix(
    cfg, type_name_to_id: Dict[str, int], margin: float
) -> np.ndarray:
    """Build the symmetric matrix of minimum allowed centre-centre distances.

    contact[i, j] = r_i + r_j + margin for every type pair that carries a
    hard_sphere force in cfg.pairs, and 0 for every pair that does not (those
    are unconstrained at placement time, e.g. ion-wall pairs handled by
    hard_wall). The exclude_intermol flag is irrelevant here: intramolecular
    pairs are never tested, because molecule geometry is rigid.
    """
    n_types = max(type_name_to_id.values()) + 1 if type_name_to_id else 0
    contact = np.zeros((n_types, n_types), dtype=float)

    for pair in cfg.get("pairs", []) or []:
        pair_types = pair.get("types")
        if not pair_types:
            continue
        for fcfg in pair.get("forces", []) or []:
            if isinstance(fcfg, str):
                continue
            if str(fcfg.get("kind", "")).lower() not in _HS_FORCE_KINDS:
                continue
            r1 = fcfg.get("r1")
            r2 = fcfg.get("r2")
            if r1 is None or r2 is None:
                continue
            d = float(r1) + float(r2) + margin
            for name1, name2 in parse_type_pairs(pair_types):
                i = type_name_to_id.get(name1)
                j = type_name_to_id.get(name2)
                if i is None or j is None:
                    continue
                d_ij = max(contact[i, j], d)
                contact[i, j] = d_ij
                contact[j, i] = d_ij
...
def _build_z_limits(
    cfg, type_name_to_id: Dict[str, int], H: float
) -> Tuple[np.ndarray, np.ndarray]:
    """Per-type allowed z interval from the hard_wall one-body forces.
    ...
    z_hi = np.full(n_types, H, dtype=float)

    for entry in cfg.get("one_body", []) or []:
        type_spec = entry.get("type")
        if type_spec is None:
            continue
        names = _parse_one_body_type_names(type_spec)
        forces_list = entry.get("forces", []) or []
        if not isinstance(forces_list, (list, tuple, ListConfig)):
        ...
        for fcfg in forces_list:
            ...
            if not fcfg.get("enabled", True):
                continue
            if str(fcfg.get("kind", "")).lower() != "hard_wall":
                continue
            r_wall = fcfg.get("r_wall")
            if r_wall is None:
                continue
            r_wall = float(r_wall)
            style = str(fcfg.get("style", "")).lower()
            for name in names:
                t = type_name_to_id.get(name)
                if t is None:
                    continue
                if style == "bottom":
...
class _NeighborBuffer:
    """Growable array of already placed atoms relevant for overlap tests."""

    def __init__(self, capacity: int = 1024):
        self._pos = np.zeros((max(capacity, 8), 3), dtype=np.float64)
        self._types = np.zeros(max(capacity, 8), dtype=np.int64)
        self.n = 0
    ...
    def _grow(self, needed: int) -> None:
        ...
        while cap < needed:
            cap *= 2
        new_pos = np.zeros((cap, 3), dtype=np.float64)
        new_types = np.zeros(cap, dtype=np.int64)
        new_pos[: self.n] = self._pos[: self.n]
        new_types[: self.n] = self._types[: self.n]
        self._pos = new_pos
        self._types = new_types

    def add(self, position: np.ndarray, type_id: int) -> None:
        if self.n + 1 > self._pos.shape[0]:
            self._grow(self.n + 1)
        self._pos[self.n] = position
        self._types[self.n] = type_id
        self.n += 1
    ...
    def types(self) -> np.ndarray:
        return self._types[: self.n]


def _init_random_hs_configuration(
    system: System,
    processed_specs: List[Tuple[str, dict, int]],
    type_name_to_id: Dict[str, int],
    cfg,
):
    """Random sequential placement respecting the actual hard-sphere force field.

    Differences from the legacy "random" mode:
      * the minimum separation is per type pair (r_i + r_j + margin), taken from
        the hard_sphere entries of cfg.pairs, instead of one global value;
      * the allowed z interval is per atom type, taken from the hard_wall
        one-body forces, instead of one global padding;
      * bulky molecules (dipoles) are inserted first, then the remaining
        particles from the largest radius down;
      * a molecule that cannot be placed raises instead of being silently
        dropped.

    Only the initial configuration changes; the Hamiltonian is untouched.
    """
    import time
    ...
    init_cfg = cfg.system.non_static_init if cfg is not None else {}
    margin = float(init_cfg.get("margin", 1e-3))
    max_attempts = int(init_cfg.get("max_attempts", 100000))

    contact = _build_hs_contact_matrix(cfg, type_name_to_id, margin)
    contact_sq = contact**2
    z_lo, z_hi = _build_z_limits(cfg, type_name_to_id, H)

    id_to_name = {v: k for k, v in type_name_to_id.items()}
    print("\n====== RANDOM_HS INITIAL PLACEMENT ======")
    print(f"margin: {margin} nm, max_attempts: {max_attempts}")
    print("Contact distances (nm), 0 = unconstrained:")
    n_types = contact.shape[0]
    for i in range(n_types):
        row = ", ".join(
            f"{id_to_name.get(j, j)}={contact[i, j]:.4f}"
            for j in range(n_types)
            if contact[i, j] > 0.0
        )
        print(f"  {id_to_name.get(i, i)}: {row if row else '(none)'}")
    print("Allowed z range per type (nm):")
    for i in range(n_types):
        print(f"  {id_to_name.get(i, i)}: [{z_lo[i]:.4f}, {z_hi[i]:.4f}]")

    # Types that participate in at least one hard-sphere pair; atoms of any
    # other type (the static wall atoms) can never block a placement.
    relevant = contact.max(axis=1) > 0.0

    # Seed the neighbour buffer with already placed atoms (static walls etc.)
    neighbors = _NeighborBuffer(capacity=1024)
    for idx in range(system.N_atoms):
        t = int(system.types[idx])
        if t < n_types and relevant[t]:
            neighbors.add(system.positions[idx], t)
    if neighbors.n:
        print(f"Pre-existing atoms relevant for overlap tests: {neighbors.n}")

    # Insertion order: dipoles first (bulkiest), then particles by decreasing
    # hard-sphere radius.
    def _order_key(item):
        molecule_type, spec, _ = item
        name = _molecule_type_name(molecule_type, spec)
        t = type_name_to_id.get(name)
        radius = 0.5 * contact[t, t] if t is not None and contact[t, t] > 0 else 0.0
        return (0 if molecule_type == "Dipole" else 1, -radius, name)

    ordered_specs = sorted(processed_specs, key=_order_key)
    print(
        "Insertion order: "
        + ", ".join(
            f"{_molecule_type_name(mt, sp)}({c})" for mt, sp, c in ordered_specs
        )
    )

    pbc = system.pbc[np.newaxis, :]
    box_row = system.box[np.newaxis, :]
    inv_box_row = system.inv_box[np.newaxis, :]
    lo_corner = np.array([0.0, 0.0, 0.0])
    extent = np.array([box[0], box[1], H])

    total_attempts = 0
    t_start = time.time()

    for molecule_type, spec, count in ordered_specs:
        label = _molecule_type_name(molecule_type, spec)
        spec_attempts = 0
        t_spec = time.time()

        for _ in tqdm(range(count), desc=f"place {label}"):
            placed = False
            attempts = 0

            while attempts < max_attempts:
                attempts += 1
                position = lo_corner + np.random.random(3) * extent
                molecule = _create_molecule(
                    position, molecule_type, spec, type_name_to_id
                )
                particles = molecule.get_particles()

                # 1) every atom of the molecule inside its own hard-wall slab
                ok = True
                for part_pos, part_type, *_ in particles:
                    z = part_pos[2]
                    if z < z_lo[part_type] or z > z_hi[part_type]:
                        ok = False
                        break
                if not ok:
                    continue

                # 2) no hard-sphere overlap with any already placed atom
                if neighbors.n > 0:
                    nb_pos = neighbors.positions
                    nb_types = neighbors.types
                    for part_pos, part_type, *_ in particles:
                        thr_sq = contact_sq[part_type][nb_types]
                        dr = nb_pos - part_pos[np.newaxis, :]
                        dr = np.where(
                            pbc, dr - box_row * np.round(dr * inv_box_row), dr
                        )
                        d_sq = np.einsum("ij,ij->i", dr, dr)
                        if np.any(d_sq < thr_sq):
                            ok = False
                            break
                    if not ok:
                        continue
                ...
                system.add_molecule(
                    molecule, type_name=_molecule_type_name(molecule_type, spec)
                )
                for part_pos, part_type, *_ in particles:
                    if part_type < n_types and relevant[part_type]:
                        neighbors.add(part_pos, part_type)
                placed = True
                break
            ...
            if not placed:
                raise RuntimeError(
                    f"random_hs: failed to place molecule '{label}' "
                    f"({molecule_type}) after {max_attempts} attempts. "
                    f"{neighbors.n} atoms already placed. The requested density "
                    f"is likely above what random sequential insertion can reach."
                )

        total_attempts += spec_attempts
        dt = time.time() - t_spec
        print(
            f"  {label}: {count} molecules, {spec_attempts} trials "
            f"({spec_attempts / max(count, 1):.1f} per molecule), {dt:.1f} s"
        )

    print(
        f"Placement done: {system.N_atoms} atoms, {total_attempts} trials, "
        f"{time.time() - t_start:.1f} s"
    )
    _verify_hs_configuration(system, contact, z_lo, z_hi, type_name_to_id)
    print("==========================================\n")


def _verify_hs_configuration(
    system: System,
    contact: np.ndarray,
    z_lo: np.ndarray,
    z_hi: np.ndarray,
    ...
    """Assert that no hard-sphere pair overlaps and every atom is in its slab."""
    N = system.N_atoms
    if N == 0:
        return

    id_to_name = {v: k for k, v in type_name_to_id.items()}
    pos = system.positions[:N]
    types = system.types[:N].astype(np.int64)
    mol_ids = system.molecule_ids[:N]
    n_types = contact.shape[0]

    # z slabs
    bad_z = (pos[:, 2] < z_lo[types] - 1e-12) | (pos[:, 2] > z_hi[types] + 1e-12)
    n_bad_z = int(np.count_nonzero(bad_z))

    # pairwise minimum distance per type pair (intramolecular pairs skipped)
    min_dist = np.full((n_types, n_types), np.inf)
    n_overlaps = 0
    worst_gap = np.inf
    pbc = system.pbc[np.newaxis, np.newaxis, :]
    box_row = system.box[np.newaxis, np.newaxis, :]
    inv_box_row = system.inv_box[np.newaxis, np.newaxis, :]

    for start in range(0, N, chunk):
        stop = min(start + chunk, N)
        dr = pos[np.newaxis, :, :] - pos[start:stop, np.newaxis, :]
        dr = np.where(pbc, dr - box_row * np.round(dr * inv_box_row), dr)
        d = np.sqrt(np.einsum("ijk,ijk->ij", dr, dr))

        t_block = types[start:stop]
        thr = contact[t_block[:, np.newaxis], types[np.newaxis, :]]
        same_mol = mol_ids[start:stop, np.newaxis] == mol_ids[np.newaxis, :]
        active = (thr > 0.0) & ~same_mol

        n_overlaps += int(np.count_nonzero(active & (d < thr)))
        if np.any(active):
            worst_gap = min(worst_gap, float(np.min(np.where(active, d - thr, np.inf))))
            d_active = np.where(active, d, np.inf)
            for t1 in np.unique(t_block):
                rows = np.where(t_block == t1)[0]
                for t2 in range(n_types):
                    if contact[t1, t2] <= 0.0:
                        continue
                    cols = np.where(types == t2)[0]
                    if cols.size == 0:
                        continue
                    val = float(np.min(d_active[np.ix_(rows, cols)]))
                    if val < min_dist[t1, t2]:
                        min_dist[t1, t2] = val
                        min_dist[t2, t1] = val

    print("Verification of the initial configuration:")
    print(f"  atoms outside their hard-wall slab: {n_bad_z}")
    print(f"  hard-sphere overlaps: {n_overlaps // 2}")
    for i in range(n_types):
        for j in range(i, n_types):
            if contact[i, j] > 0.0 and np.isfinite(min_dist[i, j]):
                print(
                    f"  {id_to_name.get(i, i)}-{id_to_name.get(j, j)}: "
                    f"min distance {min_dist[i, j]:.4f} nm, "
                    f"contact {contact[i, j]:.4f} nm"
                )
    if np.isfinite(worst_gap):
        print(f"  smallest gap (distance - contact): {worst_gap:.6f} nm")

    if n_bad_z or n_overlaps:
        raise RuntimeError(
            f"random_hs produced an invalid configuration: {n_bad_z} atoms outside "
            f"their slab, {n_overlaps // 2} overlapping hard-sphere pairs"
        )
...
def create_sampler(
    ...
    # Build insertable_types for muVT ensemble
    insertable_types: Dict[str, dict] = {}
    ensemble = cfg.simulation.get("ensemble", "NVT")
    if ensemble == "muVT" and system.mu is not None:
        mu_dict = system.mu if isinstance(system.mu, dict) else {}
        types_cfg = cfg.get("types", [])
        for spec_entry in types_cfg:
            if isinstance(spec_entry, DictConfig):
                spec_entry = OmegaConf.to_container(spec_entry, resolve=True)
            if "Particle" in spec_entry and spec_entry["Particle"]:
                spec = spec_entry["Particle"]
                t_name = spec.get("type", "")
                if t_name in mu_dict and not spec.get("static", False):
                    insertable_types[t_name] = {
                        "mu": float(mu_dict[t_name]),
                        "molecule_type": "Particle",
                        "spec": dict(spec),
                        "mass": float(spec.get("mass", 1.0)),
                    }
            elif "Dipole" in spec_entry and spec_entry["Dipole"]:
                spec = spec_entry["Dipole"]
                # Use type_plus as the key for the dipole type
                t_name = spec.get("type_plus", "")
                if t_name in mu_dict and not spec.get("static", False):
                    insertable_types[t_name] = {
                        "mu": float(mu_dict[t_name]),
                        "molecule_type": "Dipole",
...
def create_logger(cfg) -> Logger:
    ...
    # Determine insertable type names for muVT per-type logging
    insertable_type_names: List[str] = []
    ensemble = cfg.simulation.get("ensemble", "NVT")
    if ensemble == "muVT":
        mu_raw = cfg.simulation.get("mu")
        if mu_raw is not None:
            if isinstance(mu_raw, DictConfig):
                mu_dict = OmegaConf.to_container(mu_raw, resolve=True)
            elif isinstance(mu_raw, dict):
                mu_dict = mu_raw
            else:
                mu_dict = {}
            insertable_type_names = list(mu_dict.keys())
    ...
    return Logger(
        ...
        insertable_type_names=insertable_type_names,
    )
